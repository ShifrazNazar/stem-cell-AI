...
export function UploadModal({
  ...
  const { setAnalysisResults } = useReportStore();
  ...
  const { mutate: detectReportType } = useMutation({
    ...
    onError: (error) => {
      console.error(error);
      setError("Failed to detect report type");
      setStep("upload");
    },
  });
  ...
  const handleAnalyzeReport = () => {
    if (files.length > 0 && detectedType) {
      setStep("processing");
      uploadFile({ file: files[0], reportType: detectedType });
    }
  };
          ...
          <AnimatePresence>
            <motion.div>
              <div className="flex flex-col space-y-4 mb-4">
                <p>
                  We have detected the following report type:
                  <span className="font-semibold"> {detectedType}</span>
                </p>
                <p>Would you like to analyze this report with our AI?</p>
              </div>
          ...
          <AnimatePresence>
            <motion.div>
              <Alert className="mt-4">
                ...
                <AlertDescription>
                  Your report has been analyzed. you can now view the results
                </AlertDescription>
              </Alert>
